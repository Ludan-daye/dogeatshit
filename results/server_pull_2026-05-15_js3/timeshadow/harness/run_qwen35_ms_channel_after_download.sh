...
MODEL="/data/timeshadow/models/Qwen3.5-9B-MS"
LOG="/data/timeshadow/logs/qwen35_ms_channel_after_download.log"

echo "==== $(date -Is) waiting for Qwen3.5-9B-MS download ====" | tee -a "$LOG"
...
while true; do
  ...
  if [ "$SECONDS" -ge "$deadline" ]; then
    echo "==== $(date -Is) timeout; safetensors=$count ====" | tee -a "$LOG"
    exit 2
  fi
...
OUT="/data/timeshadow/results/channel_probe_qwen35_9b_ms_n30_$(date +%Y%m%d_%H%M%S).json"
